// Input-поле із кнопкою «Start timeout» — у інпут-полі користувач може ввести кількість відповідних секунд, які залишилися у росії та росіян до їх зникнення
/*
const input = document.querySelector('#start_seconds');
const btnTimeout = document.querySelector('#start_timeout');


//нам нужно чтобы этот функционал срабатывал при нажатии на кнопку
// а у нас сразу отрабатывает
//как нужно сделать?
...
// 1. пользователь вводит секунды в инпут поле
// 2. за то время выводится уведомление что росияне все
...
// Input-поле з типом date та кнопка Start Campaign. Користувач обирає дату. Після того, клікає Start Campaign. Задача - відобразити таймер, до дати яку обрав користувач у вигляді "До старту кампанії залишилось РОКИ:МІСЯЦІ:ГОДИНИ:ХВИЛИНИ"


// console.log(btnCampaign);
...
function calculateDiff(inputDate) {
    ...
    const Day = Hour * 24;
    const Month = Day * 30; // To simplify calculations
    const Year = Day * 365;
...
btnCampaign.addEventListener('click', function() {
    ...
    // на входе у нас строка!  а нужно Объект Дата (стобы потом сделать вычисление одной даты из другой)
    // код ниже создаёт объект Дата из строки
    inputeDate = new Date(inputeDate);

    // высчитать разницу между введённой датой и сейчас
    // чтобы не ждать минуту перед ПЕРВЫМ показом - сразу выполняем расчёт и выводим
    ...
    }, 1000);  //каждую секунду будет считать и отображать результат
})



// 3 select-поля та кнопка Calculate. Поле 1 - модель телефону. Поле 2 - кількість оперативної пам'яті. Поле 3 - обʼєм вбудованої пам'яті. При натисканні на кнопку - вивести ціну обраного телефону. Зауважте, що кожний телефон може мати всі поля, тобто уявімо, що всі
// iPhone/Samsung/Pixel/OnePlus можуть мати 2, 4, 6 та 8 ГБ оперативнох пам'яті,
